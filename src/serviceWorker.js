...
export function register(config) {
  if (import.meta.env.VITE_NODE_ENV === 'production' && 'serviceWorker' in navigator) {
    ...
    if (publicUrl.origin !== window.location.origin) {
      // Service worker won't work if VITE_PUBLIC_URL is on a different origin.
      return;
    }
    ...
    window.addEventListener('load', () => {
      ...
      if (isLocalhost) {
        ...
        navigator.serviceWorker.ready.then(() => {
          console.log('This web app is being served cache-first by a service worker. For Firebase Hosting, ensure proper configuration: https://firebase.google.com/docs/hosting');
        });
      } else {
